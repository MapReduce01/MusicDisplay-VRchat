...
def start_function():
    def fetchstart2():
        ...
        if combo.get() == '网易云':
            try:
                if netease_status == 0:
                    print('Processing 网易云 ...')
                    current_player = '网易云'
                    title = ""
                    count = 0
                    if "cloudmusic.exe" not in (p.name() for p in psutil.process_iter()):
                        CTkMessagebox(title="Info", message="网易云未运行")
                    else:
                        while (mt.title() == '辣鸡软件') and (current_player == '网易云') and ("cloudmusic.exe" in (p.name() for p in psutil.process_iter())):
                            netease_status = 1
                            Chrome_status = 0
                            Edge_status = 0
                            try:
            ...
            try:
                if Chrome_status == 0:
                    ...
                    if "chrome.exe" not in (p.name() for p in psutil.process_iter()):
                        CTkMessagebox(title="Info", message="Chrome未运行")
                    else:
                        while (mt.title() == '辣鸡软件') and (current_player == 'Chrome') and ("chrome.exe" in (p.name() for p in psutil.process_iter())):
                            netease_status = 0
                            Chrome_status = 1
                            Edge_status = 0
                            try:
                                ...
                                if (music != "All") and (title != "♫ Now playing:  " + music):
                                    ...
                                    msg[0] = f"{title}"
                                    client.send_message("/chatbox/input", msg)
                                elif (music != "All") and (count > 5 or count == 5):
                                    title = "♫ Now playing:  " + music
                                    msg[0] = f"{title}"
                                    client.send_message("/chatbox/input", msg)
                                    count = 0
            ...
            try:
                if Edge_status == 0:
                    ...
                    if "Clicknium.Web.NativeMessageHost.exe" not in (p.name() for p in psutil.process_iter()):
                        CTkMessagebox(title="Info", message="Edge未运行")
                    else:
                        while (mt.title() == '辣鸡软件') and (current_player == 'Edge') and (current_player == 'Edge') and ("Clicknium.Web.NativeMessageHost.exe" in (p.name() for p in psutil.process_iter())):
                            netease_status = 0
                            Chrome_status = 0
                            Edge_status = 1
                            try:
                ...
                if "Clicknium.Web.NativeMessageHost.exe" not in (p.name() for p in psutil.process_iter()):
                    ...
                    while (mt.title() == '辣鸡软件') and (current_player == 'Edge') and ("Clicknium.Web.NativeMessageHost.exe" in (p.name() for p in psutil.process_iter())):
                        netease_status = 0
                        Chrome_status = 0
                        Edge_status = 1
                        try:
...
def readmeclick():
    ...
    print('------------------------------------------------------------------------------'+'\n' + '若要显示Chrome页面不要将Chrome最小化'+'\n' + '\n' 
    +'使用Chrome和Edge时需把想要显示的Tab移到最前，只会读取第一个Tab'+'\n'
          + '\n' +'对于Edge，如未安装Clicknium插件，第一次运行系统会:'+'\n'+'a) Edge未启动 --- 弹command line自行安装'
          +'\n' 'b) Edge进程已存在 --- Edge弹窗确认安装'
          +'\n'+'安装插件后打开Edge, 右上角会提示启用插件，启用后重启Edge即可'+'\n'+
          '------------------------------------------------------------------------------')
...
def restart_program():
    ...
    if current_player == " ":
        ...
        print("Killing "+ current_player + " thread ...")
        current_player = " "
        print(current_player + " thread killed.")
    

def CTkMessagebox(title, message, timeout=2000):
    root = tk.Tk()
    root.title(title)
    root.withdraw()
...
mt.title("辣鸡软件")
# mt.geometry("500x300")
combo = ttk.Combobox(mt,font=("微软雅黑", 16), width=30)
combo["values"] = ("网易云", "Chrome", "Microsoft Edge")
combo.current(0)
combo.pack()

# combo = customtkinter.CTkComboBox(master = mt,
#                                      values=["网易云", "Chrome", "Microsoft Edge"], width = 300,
#                                      command=combobox_callback)
# combo.pack()
# combo.set("网易云")

startButton = tk.Button(mt, text='Start', font=("Arial", 26), command=lambda:start_function())
startButton.pack(pady=5)
restart_button = tk.Button(mt, text='Stop', font=("微软雅黑", 18), command=lambda:restart_program())
restart_button.pack()
readme = tk.Button(mt, text='Readme!!! 兼 console output', font=("微软雅黑", 12), command=lambda:readmeclick())
readme.pack(pady=5)
